...
BUILD		:=	build
SOURCES		:=	source lib/zipper/source lib/glad/source
RESOURCES	:=	resources
DATA		:=	data
INCLUDES	:=	include lib/zipper/include /lib/borealis/library/include/borealis/extern/nlohmann lib/glad/include
APP_TITLE	:=	E4T Patcher
APP_AUTHOR	:=	Anime_World
APP_VERSION :=  1.0.0
TARGET		:=	e4t-patcher
...
CXXFLAGS	:= $(CFLAGS) -std=gnu++20 -fexceptions -Wno-reorder -fpermissive

ASFLAGS	:=	-g $(ARCH)
LDFLAGS	=	-specs=$(DEVKITPRO)/libnx/switch.specs -g $(ARCH) -Wl,-Map,$(notdir $*.map)

LIBS	:= -lm -lcurl -lnx -lz -lmbedtls -lmbedx509 -lmbedcrypto -lstdc++fs -lwebp
...
# glad is compiled as a source file (not -lglad), filter it out from LIBS
LIBS := $(filter-out -lglad,$(LIBS))
